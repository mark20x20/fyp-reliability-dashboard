...
class Repository:
    ...
    def create_model(self, name, arch, dropout_p, dropout_layers,
                     checkpoint_path, val_accuracy=None) -> int: ...
    def create_run(self, model_id, dataset_name, cfg, user_id=None) -> int: ...
    def update_run_status(self, run_id, status) -> None: ...
    def insert_image_result(self, run_id, image_meta, prediction, explanation) -> int: ...
    def insert_baselines(self, run_id, baselines) -> None: ...
    def upsert_risk_flags(self, run_id, flags) -> None: ...
    def insert_review(self, image_id, user_id, decision, comment=None) -> int: ...
    def fetch_metrics(self, run_id, filters=None) -> pd.DataFrame: ...
    # Supported filter columns (column__gte / column__lte / column__eq):
    #   confidence, entropy, pred_variance, pred_agreement,
    #   dataset_type, correct, risk_group,
    #   cam_iou_mean, cam_corr_mean, topk_overlap,
...
`insert_image_result` wraps the `images`, `predictions`, and `explanations` inserts in a single transaction (`with self.conn:`).  A CHECK constraint violation on any of the three therefore rolls back the whole operation, leaving no orphan rows.  TC26 asserts both that `IntegrityError` is raised and that the `images` count for the run remains zero after the failed call.
